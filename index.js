...
=======
 * @author: DSCode
 * @create: 2021-05-27 15:34 PM
 * @license: MIT
 * @lastAuthor: DSCode
 * @lastEditTime: 2021-05-27 16:04 PM
 * @desc: HAPI demo
 */
// ! 启用严格模式
'use strict'
...
server.route({
  ...
  handler: async (req) => {
    try {
      ...
      const sql =
        'SELECT id FROM `mytest`.`ceshi` WHERE username="' +
        username +
        '" AND password="' +
        password +
        '";'
      const id = await req.app.db.query(sql)
      return id
    } catch (error) {
      return { tag: 300, message: error }
    }
  },
})
...
server.route({
  ...
  handler: async (req) => {
    try {
      ...
      const username = req.payload.username
      const sql =
        'SELECT id FROM `userdb`.`user` WHERE username="' + username + ';'
      const id = await req.app.db.query(sql)
      if (id) {
        return '用户已注册'
      } else {
        const sql =
          "INSERT INTO  `mytest`.`ceshi2`(`username`,`password`) VALUES ('" +
          username +
          "','" +
          password +
          "');"
        return req.app.db.query(sql)
      }
    } catch (error) {
      return { tag: 300, message: error }
    }
